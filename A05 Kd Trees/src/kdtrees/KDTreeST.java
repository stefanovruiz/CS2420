...
public class KDTreeST<Value> {
	...
	private RectHV newRect(Node parent, Point2D p, boolean isVertical) {
		...
		if (isVertical  && compare >= 0) return new RectHV(parent.rect.xmin(), parent.point.y(), parent.rect.xmax(), parent.rect.ymax());

		if (isVertical  && compare <  0) return new RectHV(parent.rect.xmin(), parent.rect.ymin(), parent.rect.xmax(), parent.point.y());

		if (!isVertical && compare >= 0) return new RectHV(parent.point.x(), parent.rect.ymin(), parent.rect.xmax(), parent.rect.ymax());

		if (!isVertical && compare <  0) return new RectHV(parent.rect.xmin(), parent.rect.ymin(), parent.point.x(), parent.rect.ymax());
		return null;
	}
